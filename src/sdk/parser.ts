 ...
 *
 * Ported from claude-mem — same parsing logic.
 * Difference: parseObservations accepts validTypes parameter instead of
 * depending on ModeManager singleton, making it decoupled and testable.
 */
 ...
 *
 * @param text - Raw text from SDK response containing XML observation blocks
 * @param validTypes - Array of valid observation type IDs from the active mode
 * @param correlationId - Optional correlation ID for log tracing
 */
export function parseObservations(text: string, validTypes: string[], correlationId?: string): ParsedObservation[] {
  ...
  while ((match = observationRegex.exec(text)) !== null) {
    ...
    // NOTE: ALWAYS save observations - never skip.
    // All fields except type are nullable in schema
    // If type is missing or invalid, use first type from mode as fallback

    // Determine final type using valid types from active mode
...
export function parseSummary(text: string, sessionId?: number): ParsedSummary | null {
  ...
  // NOTE: 100% of the time we must SAVE the summary, even if fields are missing.
  // All fields are nullable — save whatever we have.
 ...
 *
 * Uses non-greedy match to handle nested tags and code snippets
 */
 ...
 * Handles nested tags and code snippets
 */
