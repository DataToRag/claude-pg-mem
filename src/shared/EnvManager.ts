...
// project .env files to set ANTHROPIC_API_KEY which the SDK auto-discovers,
...
const BLOCKED_ENV_VARS = [
  'ANTHROPIC_API_KEY',  // Prevent auto-discovery from project .env files
...
export interface ClaudePgMemoryEnv {
...
export function loadClaudePgMemoryEnv(): ClaudePgMemoryEnv {
  ...
  try {
    ...
    const result: ClaudePgMemoryEnv = {};
 ...
 * only ANTHROPIC_API_KEY to prevent accidental billing from project .env files.
 *
...
export function getCredential(key: keyof ClaudePgMemoryEnv): string | undefined {
  const env = loadClaudePgMemoryEnv();
  return env[key];
}
