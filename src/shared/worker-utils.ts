 ...
 * AbortSignal.timeout() can cause issues in some runtimes,
 ...
 * - Hooks have 15-second timeout, but full initialization can take 5+ minutes
 ...
 * Note: Auto-restart on version mismatch is handled in worker-service start command.
 * This function logs for informational purposes only.
 * Skips comparison when either version is 'unknown' (avoids restart loops).
 */
async function checkWorkerVersion(): Promise<void> {
  try {
    ...
    if (pluginVersion !== workerVersion) {
      // Just log debug info - auto-restart handles the mismatch in worker-service
