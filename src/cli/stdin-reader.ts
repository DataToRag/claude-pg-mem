...
// so stdin.on('end') never fires and hooks hang indefinitely.
//
 ...
 *
 * Safely checks if stdin is usable. If stdin is a TTY,
 * we're running interactively (not from Claude Code hook).
 */
function isStdinAvailable(): boolean {
  try {
    ...
    // Accessing stdin.readable triggers lazy initialization.
    // If we get here without throwing, stdin is available.
    // Note: Node doesn't reliably set it to false.
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions
    stdin.readable;
    return true;
  } catch {
    // Crashed trying to access stdin
...
export async function readJsonFromStdin(): Promise<unknown> {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // If attaching listeners fails, resolve with undefined
      resolved = true;
      clearTimeout(safetyTimeoutId);
      cleanup();
      resolve(undefined);
