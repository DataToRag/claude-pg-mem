...
export type Component = 'HOOK' | 'WORKER' | 'SDK' | 'PARSER' | 'DB' | 'SYSTEM' | 'HTTP' | 'SESSION' | 'EMBED' | 'QUEUE' | 'PG' | 'ENV' | 'FOLDER_INDEX' | 'CLAUDE_MD' | 'VIEWER' | 'MCP';
...
class Logger {
   ...
   * Uses direct file reading to avoid circular dependency
   */
  ...
  success(component: Component, message: string, context?: LogContext, data?: any): void {
    this.info(component, `OK ${message}`, context, data);
  }
  ...
  failure(component: Component, message: string, context?: LogContext, data?: any): void {
    this.error(component, `FAIL ${message}`, context, data);
  }
  ...
  timing(component: Component, message: string, durationMs: number, context?: LogContext): void {
    this.info(component, `TIME ${message}`, context, { duration: `${durationMs}ms` });
  }
