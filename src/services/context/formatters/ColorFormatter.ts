 ...
 * Ported from claude-mem — removed ModeManager dependency, uses static type icons.
 */
...
import { formatObservationTokenDisplay, getTypeIcon } from '../TokenCalculator.js';
...
export function renderColorLegend(): string[] {
  const typeLegend = 'B bugfix | F feature | R refactor | D discovery | X decision | C change';
  return [
    `${colors.dim}Legend: session-request | ${typeLegend}${colors.reset}`,
    '',
  ];
}
...
export function renderColorColumnKey(): string[] {
  return [
    ...
    `${colors.dim}  Work: Tokens spent on work that produced this record${colors.reset}`,
    '',
  ];
}
...
export function renderColorContextIndex(): string[] {
  return [
    ...
    `${colors.dim}  - Search history: Use the search tool for past decisions, bugs, and deeper research${colors.reset}`,
...
export function renderColorTableRow(
  ...
  const { readTokens, discoveryTokens, workEmoji } = formatObservationTokenDisplay(
    obs,
    config,
  );
...
export function renderColorFooter(
  ...
  return [
    '',
    `${colors.dim}Access ${workTokensK}k tokens of past research & decisions for just ${totalReadTokens.toLocaleString()}t. Use the search tool to access memories by ID.${colors.reset}`,
  ];
}
