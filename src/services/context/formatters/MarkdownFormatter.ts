...
export function renderMarkdownLegend(): string[] {
  ...
  return [`**Legend:** session-request | ${typeLegend}`, ''];
}
...
export function renderMarkdownContextIndex(): string[] {
  return [
    ...
    `- Search history: Use the search tool for past decisions, bugs, and deeper research`,
...
export function renderMarkdownFooter(
  ...
  return [
    '',
    `Access ${workTokensK}k tokens of past research & decisions for just ${totalReadTokens.toLocaleString()}t. Use the search tool to access memories by ID.`,
  ];
}
