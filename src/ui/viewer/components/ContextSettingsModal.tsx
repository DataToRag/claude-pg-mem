          ...
          <div className="settings-column">
            ...
            >
              ...
              >
                <input
                  ...
                  min="1"
                  max="200"
                  value={formState.CLAUDE_PG_MEM_CONTEXT_OBSERVATIONS || '50'}
                  onChange={(e) => updateSetting('CLAUDE_PG_MEM_CONTEXT_OBSERVATIONS', e.target.value)}
                />
              </FormField>
              <FormField
                label="Sessions"
              ...
              >
                <input
                  ...
                  min="1"
                  max="50"
                  value={formState.CLAUDE_PG_MEM_CONTEXT_SESSION_COUNT || '10'}
                  onChange={(e) => updateSetting('CLAUDE_PG_MEM_CONTEXT_SESSION_COUNT', e.target.value)}
                />
              </FormField>
            </CollapsibleSection>
            ...
            >
              <ChipGroup
                ...
                selectedValues={getArrayValues('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_TYPES')}
                onToggle={(value) => toggleArrayValue('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_TYPES', value)}
                onSelectAll={() => setAllArrayValues('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_TYPES', observationTypes)}
                onSelectNone={() => setAllArrayValues('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_TYPES', [])}
              />
              <ChipGroup
                label="Concept"
                options={observationConcepts}
                selectedValues={getArrayValues('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_CONCEPTS')}
                onToggle={(value) => toggleArrayValue('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_CONCEPTS', value)}
                onSelectAll={() => setAllArrayValues('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_CONCEPTS', observationConcepts)}
                onSelectNone={() => setAllArrayValues('CLAUDE_PG_MEM_CONTEXT_OBSERVATION_CONCEPTS', [])}
              />
            </CollapsibleSection>
              ...
              <div className="display-subsection">
                ...
                >
                  <input
                    ...
                    min="0"
                    max="20"
                    value={formState.CLAUDE_PG_MEM_CONTEXT_FULL_COUNT || '5'}
                    onChange={(e) => updateSetting('CLAUDE_PG_MEM_CONTEXT_FULL_COUNT', e.target.value)}
                  />
                </FormField>
                <FormField
                  label="Field"
                ...
                >
                  <select
                    value={formState.CLAUDE_PG_MEM_CONTEXT_FULL_FIELD || 'narrative'}
                    onChange={(e) => updateSetting('CLAUDE_PG_MEM_CONTEXT_FULL_FIELD', e.target.value)}
                  >
              ...
              <div className="display-subsection">
                ...
                <div className="toggle-group">
                  <ToggleSwitch
                    ...
                    checked={formState.CLAUDE_PG_MEM_CONTEXT_SHOW_READ_TOKENS === 'true'}
                    onChange={() => toggleBoolean('CLAUDE_PG_MEM_CONTEXT_SHOW_READ_TOKENS')}
                  />
                  <ToggleSwitch
                    id="show-work-tokens"
                    label="Work investment"
                    description="Tokens spent creating this observation"
                    checked={formState.CLAUDE_PG_MEM_CONTEXT_SHOW_WORK_TOKENS === 'true'}
                    onChange={() => toggleBoolean('CLAUDE_PG_MEM_CONTEXT_SHOW_WORK_TOKENS')}
                  />
                  <ToggleSwitch
                    id="show-savings-amount"
                    label="Savings"
                    description="Total tokens saved by reusing context"
                    checked={formState.CLAUDE_PG_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT === 'true'}
                    onChange={() => toggleBoolean('CLAUDE_PG_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT')}
                  />
                </div>
              </div>
            </CollapsibleSection>
            ...
            >
              ...
              >
                <select
                  value={formState.CLAUDE_PG_MEM_PROVIDER || 'claude'}
                  onChange={(e) => updateSetting('CLAUDE_PG_MEM_PROVIDER', e.target.value)}
                >
              ...
              {formState.CLAUDE_PG_MEM_PROVIDER === 'claude' && (
                <FormField
                  label="Claude Model"
                  tooltip="Claude model used for generating observations"
                >
                  <select
                    value={formState.CLAUDE_PG_MEM_MODEL || 'haiku'}
                    onChange={(e) => updateSetting('CLAUDE_PG_MEM_MODEL', e.target.value)}
                  >
              ...
              {formState.CLAUDE_PG_MEM_PROVIDER === 'gemini' && (
                <>
                  <FormField
                  ...
                  >
                    <input
                      type="password"
                      value={formState.CLAUDE_PG_MEM_GEMINI_API_KEY || ''}
                      onChange={(e) => updateSetting('CLAUDE_PG_MEM_GEMINI_API_KEY', e.target.value)}
                  ...
                  >
                    <select
                      value={formState.CLAUDE_PG_MEM_GEMINI_MODEL || 'gemini-2.5-flash-lite'}
                      onChange={(e) => updateSetting('CLAUDE_PG_MEM_GEMINI_MODEL', e.target.value)}
                    >
                  ...
                  <div className="toggle-group" style={{ marginTop: '8px' }}>
                    <ToggleSwitch
                      ...
                      checked={formState.CLAUDE_PG_MEM_GEMINI_RATE_LIMITING_ENABLED === 'true'}
                      onChange={(checked) => updateSetting('CLAUDE_PG_MEM_GEMINI_RATE_LIMITING_ENABLED', checked ? 'true' : 'false')}
                    />
                  </div>
                </>
              )}

              {formState.CLAUDE_PG_MEM_PROVIDER === 'openrouter' && (
                <>
                  <FormField
                  ...
                  >
                    <input
                      type="password"
                      value={formState.CLAUDE_PG_MEM_OPENROUTER_API_KEY || ''}
                      onChange={(e) => updateSetting('CLAUDE_PG_MEM_OPENROUTER_API_KEY', e.target.value)}
                  ...
                  >
                    <input
                      type="text"
                      value={formState.CLAUDE_PG_MEM_OPENROUTER_MODEL || 'xiaomi/mimo-v2-flash:free'}
                      onChange={(e) => updateSetting('CLAUDE_PG_MEM_OPENROUTER_MODEL', e.target.value)}
                  ...
                  >
                    <input
                      type="text"
                      value={formState.CLAUDE_PG_MEM_OPENROUTER_SITE_URL || ''}
                      onChange={(e) => updateSetting('CLAUDE_PG_MEM_OPENROUTER_SITE_URL', e.target.value)}
                  ...
                  >
                    <input
                      type="text"
                      value={formState.CLAUDE_PG_MEM_OPENROUTER_APP_NAME || 'claude-pg-mem'}
                      onChange={(e) => updateSetting('CLAUDE_PG_MEM_OPENROUTER_APP_NAME', e.target.value)}
                      placeholder="claude-pg-mem"
                    />
                  </FormField>
                </>
              )}
              ...
              >
                <input
                  ...
                  min="1024"
                  max="65535"
                  value={formState.CLAUDE_PG_MEM_WORKER_PORT || '37778'}
                  onChange={(e) => updateSetting('CLAUDE_PG_MEM_WORKER_PORT', e.target.value)}
                />
              </FormField>
              ...
              <div className="toggle-group" style={{ marginTop: '12px' }}>
                <ToggleSwitch
                  ...
                  checked={formState.CLAUDE_PG_MEM_CONTEXT_SHOW_LAST_SUMMARY === 'true'}
                  onChange={() => toggleBoolean('CLAUDE_PG_MEM_CONTEXT_SHOW_LAST_SUMMARY')}
                />
                <ToggleSwitch
                  id="show-last-message"
                  label="Include last message"
                  description="Add previous session's final message"
                  checked={formState.CLAUDE_PG_MEM_CONTEXT_SHOW_LAST_MESSAGE === 'true'}
                  onChange={() => toggleBoolean('CLAUDE_PG_MEM_CONTEXT_SHOW_LAST_MESSAGE')}
                />
              </div>
            </CollapsibleSection>
          </div>
